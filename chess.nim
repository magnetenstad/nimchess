...
while true:
    ...
    if input.len == 0:
        ...
        echo "thinking.."
        let depth = 3
        let evaluation = evaluate_recursive(chess, depth, turn)
        echo evaluation
        if not evaluation.isEmpty():
            move_piece(chess, evaluation.moveFrom, evaluation.moveTo)
        ...
        var moves = get_moves(chess, piece, a[0], a[1])
        if not moves.contains(b):
            echo "Not a legal move."
        move_piece(chess, a, b)
        turn = not turn
    echo "Current evaluation: ", evaluate(chess), "\n"
    echo draw(chess)
